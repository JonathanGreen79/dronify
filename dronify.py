# dronify.py — streamlined with flag badges + improved Operator ID display
...
/* flag & operator badges */
.badge-row { margin:.55rem 0 .2rem 0; }
.badge-flag {
  display:inline-flex; align-items:center; gap:.45rem;
  padding:4px 10px; border-radius:12px;
  background:#F3F4F6; color:#111827; font-weight:700; font-size:.82rem;
  box-shadow: inset 0 0 0 1px #E5E7EB;
}
.badge-flag .emoji { font-size:1rem; line-height:1; }
.badge-op {
  display:inline-flex; align-items:center; gap:.45rem;
  padding:6px 12px; border-radius:999px; font-weight:700; font-size:.82rem;
  color:#111827;
}
.badge-op.required   { background: #FEE2E2; color:#7F1D1D; box-shadow: inset 0 0 0 1px #FCA5A5; }
.badge-op.optional   { background: #DCFCE7; color:#14532D; box-shadow: inset 0 0 0 1px #86EFAC; }
.badge-op.unknown    { background: #E5E7EB; color:#374151; box-shadow: inset 0 0 0 1px #D1D5DB; }
.badge-op .emoji     { font-size:1rem; line-height:1; }
</style>
...
if not segment:
    ...
    if model:
        ...
        if not sel.empty:
            ...
            # --- new fancy EU/UK/Operator display ---
            eu = (row.get("eu_class_marking") or row.get("class_marking") or "unknown")
            uk = (row.get("uk_class_marking") or row.get("class_marking") or "unknown")
            op_raw = str(row.get("operator_id_required", "")).strip().lower()

            if op_raw in ("yes", "true", "1"):
                op_html = "<div class='badge-op required'><span class='emoji'>🪪</span>Operator ID: Required</div>"
            elif op_raw in ("no", "false", "0"):
                op_html = "<div class='badge-op optional'><span class='emoji'>✅</span>Operator ID: Not Required</div>"
            else:
                op_html = "<div class='badge-op unknown'><span class='emoji'>❔</span>Operator ID: Unknown</div>"

            st.sidebar.markdown(
                f"<div class='badge-row'>"
                f"<div class='badge-flag'><span class='emoji'>🇪🇺</span> EU: {eu}</div><br>"
                f"<div class='badge-flag'><span class='emoji'>🇬🇧</span> UK: {uk}</div><br>"
                f"{op_html}</div>",
                unsafe_allow_html=True
            )
